...
async sendMessage(query, file = null) {
    try {
        ...
        // Build headers: keep your existing Authorization,
        // but remove Content-Type so axios/browser sets the multipart boundary.
        const headers = this.getHeaders ? { ...this.getHeaders() } : {};
        delete headers["Content-Type"];

        // If your axios instance has baseURL, use the relative path:
        const res = await axios.post(
            `/chat/sessions/${this.sessionId}/messages`,
            formData,
            { headers }
        );
...
async getChatHistory(limit = 50) {
    try {
        ...
        const response = await axios.get(
            `/chat/sessions/${this.sessionId}/history?limit=${limit}`,
            { headers: this.getHeaders() }
        );
